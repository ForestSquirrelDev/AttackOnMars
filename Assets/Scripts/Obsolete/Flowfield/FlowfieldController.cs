...
using static Utils.Pathfinding.FlowfieldUtility;
...
namespace Obsolete {
    public class FlowfieldController : MonoBehaviour {
        ...
        private void GenerateChildGrid(Vector3 target, FlowFieldCell parentCell, FlowFieldCell targetParentCell, float childCellSize, IList<FlowFieldCell> parentCells, int2 parentCellsGridSize) {
            ...
            if (IsOutOfRange(parentCells.Count, bestParentIndex)) {
        ...
        private void GenerateMergedGrid(float cellSize) {
            ...
            var terrainRect = TerrainUtility.GetWorldRect(_terrain.transform.position.x, _terrain.transform.position.z,
                _terrain.terrainData.size.x, _terrain.terrainData.size.z);
...
namespace Flowfield {
    ...
    // This file was supposed to be test run of flowfield algorithm, so that it would be easier to implement it in ECS after i know how to make it in classic monobehaviour style.
    // Now that ECS implementation is ready, this one goes obsolete.
}